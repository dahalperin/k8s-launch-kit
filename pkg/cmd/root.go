...
var rootCmd = &cobra.Command{
	...
	Long: `l8k is a CLI tool for generating and deploying NVIDIA cloud-native configurations on Kubernetes clusters.

The tool operates in 3 phases:

1. DISCOVER CLUSTER CONFIG: Deploy a thin profile of the Network Operator to discover 
   the cluster configuration and capabilities by using --discover-cluster-config.
   This phase can be skipped if you provide your own configuration with --user-config.
   This phase requires --kubeconfig.
